...
class Board:
    ...
    def on_discard(self, player, is_tutorial_winning_tile=False):
        ...
        for index in range(len(self.players)):
            ...
            if self.players[index].name != "Player":
                pass
            on_left = (
                player_index + 1 == index if index != 0 else player_index - 3 == index
            )
            ron_available = False
            if is_tutorial_winning_tile:
                ron_available = True
            if self.players[index].on_opponent_discard(
                player, on_left, ron_available=ron_available
            ):
    ...
    def on_decision(self, call, player, discarder):
        ...
        if call == Call.Pon:
            ...
            player.melded_hand += [
                Meld(
                    [pon_tile, pon_tile, pon_tile],
                    CallDirection.get_call_direction(
                        self.players.index(player), self.players.index(discarder)
                    ),
                )
            ]
            removed = 0
        ...
        if call == Call.Chi:
            ...
            direction = CallDirection.get_call_direction(
                self.players.index(player), self.players.index(discarder)
            )
            if direction == 1:
                player.melded_hand += [
                    Meld([chi_tile, meld_peice1, meld_peice2], direction)
                ]
            elif direction == 2:
                player.melded_hand += [
                    Meld([meld_peice1, chi_tile, meld_peice2], direction)
                ]
            elif direction == 3:
                player.melded_hand += [
                    Meld([meld_peice1, meld_peice2, chi_tile], direction)
                ]
